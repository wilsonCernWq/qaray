...
class Texture : public ItemBase {
 public:
  ...
  virtual Color3f Sample(const Point3 &uvw, const Point3 duvw[2], bool elliptic = true) const {
...
class TexturedColor {
 ...
 public:
  ...
  Color3f Sample(const Point3 &uvw) const { return (map) ? Color3f(color * map->Sample(uvw)) : color; }

  Color3f Sample(const Point3 &uvw, const Point3 duvw[2], bool elliptic = true) const {
    return (map) ? Color3f(color * map->Sample(uvw, duvw, elliptic)) : color;
  }

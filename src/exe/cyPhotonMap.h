...
class PhotonMap {
 private:
  ...
  using Color = qaray::Color3f;
  using ColorA = qaray::Color4f;
  using Color24 = qaray::Color3c;
 public:
  ...
  class Photon {
   ...
   public:
    ...
    void GetPower(Color &c) const { c = qaray::ToColor(color) * power; }
...
inline void PhotonMap::LocatePhotons(NearestPhotons &np, const int index) const
{
  ...
  if (dist2 < np.dist2[0]) {
    ...
    if (np.normal) {
      Point3f dir;
      p.GetDirection(dir);
      if (qaray::dot(dir, (*np.normal)) >= 0) return;
      if (np.normScale > 0) {
        float perp = qaray::dot(dif, (*np.normal));
